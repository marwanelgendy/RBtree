    ...
    private void removeDoubleBlack ( INode<T,V> current , boolean isNull ){
        ...
        if(w != null && w.getColor() == INode.RED ){
            if(current == current.getParent().getLeftChild()){
                ...
                if(isNull){
                    w.getLeftChild().setLeftChild(null);
                    w.getLeftChild().getRightChild().setColor(INode.RED);
                }
                else{
                    w.getLeftChild().setColor(INode.RED);
                    INode<T,V> h = getSibling(current);
                    if( h != null && h.getColor() == INode.BLACK && h.getRightChild() != null && h.getRightChild().getColor()== INode.BLACK && h.getLeftChild() != null && h.getRightChild().getColor() == INode.BLACK){
                        if(current.getRightChild() != null)
                            current.getRightChild().setColor(INode.BLACK);
                        else
                            current.getLeftChild().setColor(INode.BLACK);
                    }
                }
            }
            else{
                ...
                w.setColor(INode.BLACK);
                if(isNull){
                    w.getRightChild().setRightChild(null);
                    w.getRightChild().getLeftChild().setColor(INode.RED);
                }
                else{
            ...
            w.setColor(INode.RED);
            if(isNull){
                if(w == w.getParent().getRightChild() )
                    w.getParent().setLeftChild(null);
                else if ( w == w.getParent().getLeftChild())
